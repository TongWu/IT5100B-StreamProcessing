...
public class ImmutableMap<K, V> {
    ...
    /**
     * Performs a reduction over each key-value pair in this map.
     * @param init The initial value of reduction
     * @param f The function that reduces over each key-value pair in this map.
     * @return The result of reduction over the whole map.
     * @param <T> The type of the initial value and the result.
     */
    public <T> T reduceEntries(T init, BiFunction<? super T, ? super Map.Entry<K, V>, ? extends T> f) {
        T res = init;
        for (Map.Entry<K, V> e : this.mp.entrySet()) {
            res = f.apply(res, e);
        }
        return res;
    }
